...
(function ($) {
    $.extend({
        ...
        modal: {
            ...
            openReturn: function (title, url, width, height, callback) {
                // 如果是移动端，就使用自适应大小弹窗
                if ($.common.isMobile()) {
                    width = 'auto';
                    height = 'auto';
            ...
            rsopen: function (title, url, width, height, data, callback) {
                // 如果是移动端，就使用自适应大小弹窗
                if ($.common.isMobile()) {
                    width = 'auto';
                    height = 'auto';
        ...
        operate: {
            ...
            // 归还图书
            returnborrow: function(id) {
                table.set();
                var url = $.common.isEmpty(id) ? table.options.returnUrl : table.options.returnUrl.replace("{id}", id);
                // 对于url能不能拿到数据，
                $.modal.openReturn("归还" + table.options.modalName, url);
            },
            // 续借图书
            renewbook: function(id) {
                table.set();
                var url = $.common.isEmpty(id) ? table.options.renewUrl : table.options.renewUrl.replace("{id}", id);
                // 对于url能不能拿到数据，
            ...
            notification:function (id){
                table.set();
                $.modal.confirm("确定要订阅该图书吗？", function() {
                    var url = $.common.isEmpty(id) ? table.options.notificationUrl : table.options.notificationUrl.replace("{id}", id);
                    if (table.options.type == table_type.bootstrapTreeTable) {
                        $.operate.get(url);
                    } else {
                        var data = { "ids": id };
            ...
            // 书籍已读
            setRead : function (url) {
                const match = url.match(/\{([^}]+)\}/);
                if (match) {
                    // 如占位符的名称
                    console.log(match[1]); // 这将是 "bookName"
                } else {
                    console.log("No valid placeholder found in the path.");
                }
                var bookName = match[1]
                var data = { "bookName": bookName};
                $.operate.submit(url.replace('/'+match[0], ''), "post", "json", data);
            ...
            // 保存信息时跳转到判断页面
            saveCheck:function (url){
                table.set()
                $.modal.openReturn("查看是否逾期", url);
                $.modal.close(1)
            },
            // 保存信息 弹出结果提示框
